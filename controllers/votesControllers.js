...
// Test errors for duplicate vote 
...
exports.castVote = async (req, res, next) => {
  try {
    ...
    // Check for existing votes to prevent duplicate entries
    const existingVote = await knex('votes')
      .where({ user_id: userId, contestant_id: contestantId })
      .first();

    if (existingVote) {
      console.log(`User ${userId} has already voted for contestant ${contestantId}.`);
