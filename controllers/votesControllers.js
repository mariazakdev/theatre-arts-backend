...
exports.castVote = async (req, res, next) => {
  try {
    const { userId, contestantId, numberOfVotes, voteType } = req.body;
    ...
    if (existingVote && userExpirations.has(userId)) {
      const expirationTime = userExpirations.get(userId);
      if (expirationTime > Date.now() && voteType === "free") {
        // If the vote is not expired and it's a free vote, return error
        console.log(`User ${userId} attempted to vote again before expiration.`);
        return res.status(400).json({
          error: "User has already voted",
          message: "User can only cast a free vote once per 24 hours",
        });
      } else if (expirationTime > Date.now() && voteType === "paid") {
        // If the vote is not expired and it's a paid vote, return error
        console.log(`User ${userId} attempted to vote again before expiration.`);
        return res.status(400).json({
          error: "User has already voted",
          message: "User can only cast a paid vote once per 3 days",
        });
      }
    }

    // Reset user's expiration time based on vote type
    let expirationTime;
    if (voteType === "free") {
      expirationTime = Date.now() + (24 * 60 * 60 * 1000); // 24 hours in milliseconds
    } else if (voteType === "paid") {
      expirationTime = Date.now() + (3 * 24 * 60 * 60 * 1000); // 3 days in milliseconds
    }
