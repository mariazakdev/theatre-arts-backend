    ...
    try {
      ...
      if (!exists) {
        await knex.schema.createTable('votes_tracker', table => {
          ...
          // Remove the unique constraint on (user_id, contestant_id)
          // table.unique(['user_id', 'contestant_id']);  <- Remove this line
  
          // Add foreign key constraints to users and contestants tables
