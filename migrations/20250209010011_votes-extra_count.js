        ...
        if (!exists) {
            return knex.schema.createTable('votes_extra', function(table) {
                ...
                table.integer('contestant_id').unsigned().notNullable()
                    .references('id').inTable('contestants').onDelete('CASCADE');
                table.integer('number_of_votes').unsigned().notNullable();
                table.timestamp('created_at').defaultTo(knex.fn.now());
                table.timestamp('last_voted_at').defaultTo(knex.fn.now()); // Auto-update timestamp
            });
        }
    });
};
